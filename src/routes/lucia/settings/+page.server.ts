...
async function updatePasswordAction(event: RequestEvent) {
  ...
  if (!passwordUpdateBucket.check(event.locals.session.id.toString(), 1)) {
    return fail(429, {
      password: {
  ...
  if (!passwordUpdateBucket.consume(event.locals.session.id.toString(), 1)) {
    return fail(429, {
      password: {
        message: "Too many requests"
      }
  ...
  passwordUpdateBucket.reset(event.locals.session.id.toString());
